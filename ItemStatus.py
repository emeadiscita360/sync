...
#txt_file = f"ItemStatusDiscrepancy_{timestamp}.txt"
workfiles_folder = os.path.join(script_dir, "WorkFiles")
os.makedirs(workfiles_folder, exist_ok=True)  # Ensure the folder exists
txt_file = os.path.join(workfiles_folder, f"ItemStatusDiscrepancy_{timestamp}.txt")

# --- Save to Excel ---
# df.to_excel(excel_file, index=False)   ---de-comment if it's needed to check in excel
